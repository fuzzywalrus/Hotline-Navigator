...
This change is delivered in two phases:

### Phase 1: AEAD Control Connection
Add ChaCha20-Poly1305 as a negotiable cipher for the control connection (chat, commands, login). After this phase, Navigator can negotiate AEAD transport with servers that support it, with graceful RC4 fallback. File transfers remain unencrypted under HOPE (same as current RC4 behavior). Test servers: Hotline Central Hub, Vespernet.

### Phase 2: AEAD File Transfers (deferred)
Add AEAD encryption to HTXF file transfers when the control connection is in AEAD mode. This phase depends on Phase 1 being validated against real servers. It will be implemented as a follow-up change.
...
### New Capabilities
- `hope-chacha20-poly1305`: ChaCha20-Poly1305 AEAD cipher negotiation, HKDF key derivation, framed AEAD transport, and HMAC-SHA256 MAC support for HOPE connections

### Modified Capabilities
- `hope`: Update cipher advertisement to include CHACHA20-POLY1305 alongside RC4; add HMAC-SHA256 to the MAC algorithm preference list
- `file-transfers` (Phase 2): When the control connection uses HOPE AEAD mode, HTXF file transfers use ChaCha20-Poly1305 encryption with per-transfer derived keys (previously always unencrypted under HOPE)
...
- **File transfers (Phase 2)**: HTXF transfer initiation must check whether AEAD mode is active and derive per-transfer keys; the transfer socket gets wrapped in AEAD framing when applicable
