...
The authenticated login (Step 3) SHALL be sent in plaintext. Transport encryption keys MUST be activated immediately after sending the authenticated login but before reading the server's reply. The server encrypts its login reply.
