...
Defines TLS encrypted connection support, including modern TLS (1.2+), legacy TLS (1.0), auto-detect, per-bookmark configuration, and SNI handling.
...
Each bookmark SHALL have a `tls` boolean field (serde default: `false` for backward compatibility). The user MUST be able to toggle this field when creating or editing a bookmark.
